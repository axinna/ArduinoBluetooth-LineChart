...
class ArduinoCommunicator: NSObject { //蓝牙通讯
    ...
    private let expectedPeripheralName = "HC08Axin"
    private let expectedCharacteristicUUIDString = "FFE1"
    ...
    func read() {
        if( self.isReady ) {
            print("ArduinoCommunicator->read():When isReady = true")
            guard let characterist = self.characterist else { return }
            self.peripheral?.readValue(for: characterist)
        }else{
            print("isReady is false")
        }
    }
}
...
extension ArduinoCommunicator: CBPeripheralDelegate {
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        if( characteristic.uuid.uuidString == characteristOfInterest.uuid.uuidString ) {
            ...
            print("arduinoCommunicator->peripheral")
        }
    }
